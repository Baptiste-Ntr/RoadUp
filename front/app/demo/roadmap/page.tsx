...
function RoadmapContent() {
  ...
  useEffect(() => {
    ...
    } else if (projects.length > 0 && !selectedProjectSlug) {
      setSelectedProjectSlug(projects[0].slug);
    }
  }, [projectSlug, projects, selectedProjectSlug]);

  const currentProject = projects.find((p) => p.slug === selectedProjectSlug);
  const projectItems = currentProject ? getItemsByProject(currentProject.id) : [];
  const comments = selectedItem ? getCommentsByItem(selectedItem.id) : [];
  ...
  const handleAddComment = async (content: string) => {
    if (selectedItem) {
      createComment(selectedItem.id, content);
  ...
  const handleToggleVote = async (itemId: string) => {
    updateItem(itemId, {
      votes_count: (projectItems.find((i) => i.id === itemId)?.votes_count || 0) + 1,
    });
        ...
        <div className="flex gap-6 overflow-x-auto pb-4">
          {columns.map((column) => (
            <KanbanColumn
              key={column.id}
              column={column}
              items={getItemsByStatus(currentProject.id, column.id)}
